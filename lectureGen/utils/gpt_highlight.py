...
def _extract_sentence_boundaries(segments: list) -> list:
    """
    Build sentence boundary ranges as (start, end) timestamps.
    Prefers word-level punctuation boundaries when available, otherwise falls
    back to segment-text punctuation.
    """
    if not segments:
        return []
    ...
    # Preferred path: word-level punctuation from Whisper.
    words = []
    for seg in segments:
    ...
    if words:
        ...
        for w in words:
            ...
            if re.search(r'[.!?]["\'\)\]]*$', token):
                if we > sent_start:
                    boundaries.append((sent_start, we))
                sent_start = None

        # Flush trailing partial sentence if punctuation is missing.
        if sent_start is not None:
            last_end = float(words[-1]['end'])
            if last_end > sent_start:
                boundaries.append((sent_start, last_end))

        return boundaries

    # Fallback path: infer boundaries from segment text punctuation.
    sent_start = None
    for seg in sorted(segments, key=lambda s: float(s['start'])):
        ...
        if re.search(r'[.!?]["\'\)\]]*\s*$', text):
            if e > sent_start:
                boundaries.append((sent_start, e))
            sent_start = None
...
def _align_end_to_boundary(start_time: float, target_end: float, segments: list,
                           min_end: float = None, forward_tolerance: float = 0.5,
                           tail_padding: float = None) -> float:
    """
    Align clip end to a safe timestamp boundary (sentence end preferred).
    Used after min/max duration adjustments so we avoid mid-word truncation.
    """
    min_end = start_time if min_end is None else min_end
    tail_padding = config.CLIP_TAIL_PADDING if tail_padding is None else tail_padding

    sentence_ranges = _extract_sentence_boundaries(segments)
    if sentence_ranges:
        sentence_starts = sorted(
            s for s, _ in sentence_ranges
            if s > start_time and s >= min_end
        )
        sentence_ends = sorted(
            e for _, e in sentence_ranges
            if e > start_time and e >= min_end
        )

        forward_ends = [e for e in sentence_ends if e >= target_end]
        if forward_ends:
            chosen_end = forward_ends[0]
        else:
            chosen_end = sentence_ends[-1]

        padded_end = chosen_end + tail_padding
        next_starts = [s for s in sentence_starts if s > chosen_end]
        if next_starts:
            padded_end = min(padded_end, next_starts[0] - 0.1)
        return max(chosen_end, padded_end)

    seg_ends = sorted(
        float(s['end']) for s in segments
        if float(s['end']) > start_time and float(s['end']) >= min_end
    )
    forward_ends = [e for e in seg_ends if e >= target_end]
    if forward_ends:
        return forward_ends[0] + tail_padding

    return seg_ends[-1] + tail_padding if seg_ends else target_end + tail_padding

def _snap_to_segments(start_time: float, end_time: float, segments: list) -> tuple:
    """Snap timestamps to sentence boundaries first, then segment boundaries."""
    if not segments:
        return start_time, end_time

    sentence_ranges = _extract_sentence_boundaries(segments)
    if sentence_ranges:
        sentence_starts = [s for s, _ in sentence_ranges]
        sentence_ends = [e for _, e in sentence_ranges]

        # Start: align to nearest sentence start not too far after requested start.
        prior_starts = [s for s in sentence_starts if s <= start_time]
        if prior_starts:
            snapped_start = max(prior_starts)
        else:
            snapped_start = sentence_starts[0]

        # End: prefer the next sentence end (forward snap) so clips don't end mid-sentence.
        next_ends = [e for e in sentence_ends if e >= end_time]
        if next_ends:
            snapped_end = min(next_ends)
        else:
            snapped_end = sentence_ends[-1]
...
def _graph_mmr_selection(chunks: list, num_highlights: int,
    ...
    for idx in selected_idx:
        ...
        if e - s < min_dur:
            e = _align_end_to_boundary(s, s + min_dur, segments, min_end=s + min_dur)
        if e - s > max_dur + 10:
            e = _align_end_to_boundary(s, s + max_dur, segments, min_end=s + min_dur)
