...
function ClipCard({ highlight, index }: { highlight: Highlight; index: number }) {
  ...
  const rankBadge = medals[index] ? (
    ...
    <span className="flex h-8 w-8 items-center justify-center rounded-full border border-slate-600 bg-slate-700/80 text-sm font-semibold text-slate-200 shadow-sm shadow-black/20">
      {index + 1}
    </span>
  );
      ...
      {expanded && (
        <div className="px-4 pb-4 border-t border-gray-700 pt-3">
          <div className="flex items-center gap-2 mb-3">
            <span className="text-xs text-gray-300 font-semibold">Highlight {index + 1}</span>
          </div>
          <div className="flex items-center gap-2 mb-3">
...
export default function SportsHighlightPage() {
  ...
  const [sortMode, setSortMode]       = useState<'timeline' | 'intensity'>('timeline');
  ...
  const timelineHighlights = [...highlights].sort((a, b) => a.start_time - b.start_time);
  const intensityHighlights = [...highlights].sort((a, b) => b.energy_score - a.energy_score);
  const displayedHighlights = sortMode === 'timeline' ? timelineHighlights : intensityHighlights;
            ...
            <div className="space-y-4">
              <div className="flex items-center justify-between">
                <h2 className="text-lg font-bold text-white flex items-center gap-2">
                  ...
                  {displayedHighlights.length} Highlights — sorted by {sortMode}
                </h2>
                <div className="flex items-center gap-2">
                  <span className="text-xs text-gray-400">Sort by:</span>
                  <button
                    onClick={() => setSortMode(sortMode === 'timeline' ? 'intensity' : 'timeline')}
                    className="px-3 py-1 bg-emerald-600 hover:bg-emerald-700 text-white text-xs rounded-lg font-medium transition-colors"
                  >
                    {sortMode === 'timeline' ? '⚡ Intensity' : '🕐 Timeline'}
                  </button>
                  <span className="text-xs text-gray-500">
                    <Clock size={12} className="inline mr-1" />
                    Total: {displayedHighlights.reduce((a, h) => a + h.duration, 0).toFixed(0)}s
                  </span>
                </div>
              </div>
