...
def ensure_backend_python() -> str:
    ...
    requirements = ROOT / "sportsHighlight" / "requirements.txt"
    needs_install = False

    if not venv_python.exists():
        print(f"⚠️  Virtual environment not found at {venv_python}")
        print("    Creating sportsHighlight/venv and installing dependencies...")
        subprocess.check_call([sys.executable, "-m", "venv", str(ROOT / "sportsHighlight" / "venv")])
        needs_install = True

    probe = subprocess.run([str(venv_python), "-c", "import librosa"], capture_output=True)
...
def main():
    ...
    try:
        python_exe = ensure_backend_python()
    except subprocess.CalledProcessError as exc:
        print(f"❌ Failed to prepare sportsHighlight environment: {exc}")
        return 1
